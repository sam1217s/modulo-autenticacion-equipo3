...
/**
 * Show Bootstrap confirmation modal for logout
 */
...
async function executeLogout() {
  ...
  try {
    ...
    if (result.success) {
      ...
      Utils.Storage.clear();
      showLogoutToast('Sesión cerrada localmente');
      setTimeout(() => {
        window.location.href = 'index.html';
      }, 2000);
    }
    ...
    Utils.Storage.clear();
    showLogoutToast('Sesión cerrada');
    setTimeout(() => {
      window.location.href = 'index.html';
    }, 2000);
  } finally {
 ...
 */
function showLogoutToast(message = 'Has cerrado sesión correctamente. Redirigiendo...') {
...
/**
 * Handle logout action (DEPRECATED - use confirmLogout instead)
 */
async function logout() {
  console.warn('logout() function is deprecated. Use confirmLogout() instead.');
  confirmLogout();
}
